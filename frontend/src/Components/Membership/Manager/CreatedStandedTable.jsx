    ...
    const fetchPkgs = async () => {
      try {
        const { data } = await axios.get(
          "http://localhost:8070/package/package"
        );
        setPromos(data);
      } catch (error) {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:8070/package/package/${id}`)
          .then((response) => {
            Swal.fire({
              title: "Success",
  ...
  return (
    <div className=" flex h-full justify-center items-center" style={bgStyle}>
      <div className="bg-black/45 h-3/4 w-3/4 rounded-[50px] py-10 px-14 flex flex-col gap-y-8">
        <p className="text-4xl font-bold text-white">Created standed package</p>
        ...
        <div className="w-full">
          <div className="grid grid-cols-6 bg-blue-700 text-white">
            ...
            <div className="border-2 border-black p-3 flex justify-center">
              Edit
            </div>
            <div className="border-2 border-black p-3 flex justify-center">
              Delete
          ...
          >
            {filteredPromos.length > 0 ? (
              filteredPromos.map((prm) => (
                <div
                  className="grid grid-cols-6 bg-blue-200 text-black"
                  key={prm._id}
                >
...
export default CreatedStandedTable;
