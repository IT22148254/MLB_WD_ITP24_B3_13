    ...
    const fetchPkgs = async () => {
      try {
        const { data } = await axios.get(
          "http://localhost:8070/package/propackage"
        );
        setPromos(data);
      } catch (error) {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:8070/package/propackage/${id}`)
          .then((response) => {
            Swal.fire({
              title: "Success",
      ...
      <div className="bg-black/45 h-3/4 w-3/4 rounded-[50px] py-10 px-14 flex flex-col gap-y-8">
        ...
        <div className="flex justify-between items-center">
          <div>
            <label
              htmlFor="search"
              className="text-2xl text-white font-bold align-top mb-15 ml-10"
            >
              Search :{" "}
            </label>
          ...
          >
            {filteredPromos.length > 0 ? (
              filteredPromos.map((prm) => (
                <div
                  className="grid grid-cols-7 bg-blue-200 text-black"
                  key={prm._id}
                >
